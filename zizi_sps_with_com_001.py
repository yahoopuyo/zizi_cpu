...
def kihu_init(kihu):
    ...
    for i in range(1,len(places)):
        if places[i] != None:
            cards_at_field_num += 1

    #print(cards_at_field_num)

    #次に、表を作成する
    kihu = [[None] * ( cards_at_field_num + 5 ) for i in range(0, cards_at_field_num)]

    #プレイヤー0のカードから順に背番号を振っていく
    start = 0
    for player_num in range(0,4):
        for i in range(start, start + len(originals[player_num])):
            kihu[i][cards_at_field_num] = originals[player_num][i - start]
    ...
    for player_num in range(0,4):
        for i in range(start, start + len(originals[player_num])):
            kihu[i][cards_at_field_num + 1 + player_num] = kihu[i][cards_at_field_num]
        start += len(originals[player_num])

    return cards_at_field_num, kihu


#棋譜にそのターンの成果を記入する関数。draw関数を実行した後に呼び出すこと。
def kihu_write(turn, turn_player, drawn_card, flug_of_judge, success_card, cards_at_field_num):
    if flug_of_judge == True:
        for i in range(0, cards_at_field_num):
            if kihu[i][cards_at_field_num] == drawn_card: #もし、背番号iのカードの表の番号（リアルナンバー）が揃ったカードの引き札の番号なら
                for j in range(0, cards_at_field_num): #タテに回す
                    if kihu[i][j] == None:
                        kihu[i][j] = turn
                for j in range(0, cards_at_field_num): #ヨコに回す
                    if kihu[j][i] == None:
                        kihu[j][i] = turn
                for j in range(cards_at_field_num + 1, cards_at_field_num + 5):
                    kihu[i][j] = kihu[i][cards_at_field_num] #みんなに揃ったカードの数字がわかる
            elif kihu[i][cards_at_field_num] == success_card: #もし、背番号iのカードの表の番号（リアルナンバー）が揃った引き札のカードの相手の番号なら
                for j in range(0, cards_at_field_num): #タテに回す
                    if kihu[i][j] == None:
                        kihu[i][j] = turn
                for j in range(0, cards_at_field_num): #ヨコに回す
        ...
        for i in range(0, cards_at_field_num):
            if kihu[i][cards_at_field_num] == drawn_card:
                back_num_of_drawn_card = i
        for i in range(0, cards_at_field_num):
            if places[kihu[i][cards_at_field_num]] == turn_player: #それぞれのカードの背番号をみて、その持ち主がターンプレイヤーであるカードについて処理をする
        ...
        kihu[back_num_of_drawn_card][cards_at_field_num + 1 + turn_player] = kihu[back_num_of_drawn_card][cards_at_field_num] #今のターンプレイヤーには引いたカードの情報がわかる
        #kihu[back_num_of_drawn_card][cards_at_field_num + 1 + turn_player] = drawn_cardでも良い。
...
cards_at_field_num, kihu = kihu_init(kihu)
...
while loser_exist == False:
    ...
    if turn_player_win == True:  #もし、先のターンの人が上がったら、次に順番が回ってくる人は、何もしない。
        ...
        kihu_write(turn, turn_player, drawn_card, flug_of_judge, success_card, cards_at_field_num)
    ...
    print("kihu is below")
    for i in range(0, cards_at_field_num):
        for j in range(0, cards_at_field_num):
            if kihu[i][j] == None:
                print("None", end = "  ")
            elif kihu[i][j] == 0:
        ...
        print("    ", end  = "")
        for j in range(cards_at_field_num, cards_at_field_num + 5):
            if kihu[i][j] == None:
                print("None", end = "  ")
            else:
